...
function MobileCart() {
    ...
    const mobilesProducts = ProductsData.filter(mobile => mobile.item === 'mobiles');
    ...
    return (
        <div className="cart">
            <h1 className="cart_head">Mobile Phones</h1>
            <Slider {...settings} className="full_slider">

                {mobilesProducts.map((e) => {
                    let { id, image, desc, price, item } = e;
                    return <div className="cart_item" key={e.id}>
